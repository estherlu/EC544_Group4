...
void loop() {
 ...
 //Ultrasonic sensor 2 
  ...
  if (distanceCm2 <= 0){
    ...
    if (dist < 0) {
      Serial.print("Distance negative Lidar : " + dist);
        dist = l_0;
    }
  ...
  { distAvg=(dist+distLast)/2;
    ...
    if ((distanceCm1 > 120) && (dist < 300) && (cmnd==49) &&(dist>=90)) {
      cmnd=50; 
    ...
    if (cmnd==50 && (((distanceCm2-distanceCm1)>0 || abs(distanceCm2-distanceCm1)<5)&&((distanceCmLast2-distanceCmLast1)>0 || abs(distanceCmLast2-distanceCmLast1)<5))) {
      cmnd = 49;
      wheels.write(110);
      esc.write(77);
