...
void loop() {
  if (cmnd == 49 || cmnd == 50)
  {
    ...
    if ((dist > 5 && dist < 60) && (distLast1 > 5 && distLast1 < 60)) {
      ...
      //Turn right
      if(dist>140 && dist<250 && distLast1>140 && distLast1<250 && (abs(distanceCm - distanceCmLast) > 35) &&  cmnd == 49){
        cmnd = 50;
        esc.write(77);
        wheels.write(50);
        ...
        if (((abs(distanceCm - distanceCmLast) < 50) && (dist<6 || dist>400)) || (distanceCm < 300)) {   // BAD
          Input = distanceCm;
          PIDleft.Compute();
          esc.write(75);
        ...
        esc.write(76);
        wheels.write(110);
        if (abs(distanceCm - distanceCm2) < 5) delay(1000);
